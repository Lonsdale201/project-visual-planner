...
type PdfT = (key: TranslationKey, params?: Record<string, string | number>, fallback?: string) => string;
...
class PdfBuilder {
  ...
  drawCover(
    ...
    date: string,
    specTexts: string[] = [],
    generatedByText = 'Generated by KnitFlow',
  ) {
    ...
    this.doc.setFontSize(8);
    this.doc.setTextColor(...C_MUTED);
    this.doc.text(generatedByText, M_LEFT, PAGE_H - 10);
...
function renderBusinessSections(
  pdf: PdfBuilder,
  nodesByKind: Map<NodeKind, ProjectNode[]>,
  ...
  if (personas.length > 0) {
    pdf.sectionTitle(tt('pdf.personas', undefined, 'Personas'));
    personas.forEach((n, i) => {
      ...
      pdf.labelValue(tt('pdf.role', undefined, 'Role'), str(n.data.role), 4);
      pdf.priorityBar(tt('pdf.priority', undefined, 'Priority'), str(n.data.priority) || 'medium');
      if (str(n.data.painPoints)) {
        pdf.miniLabel(tt('pdf.painPoints', undefined, 'Pain Points'), 4);
        pdf.bodyText(str(n.data.painPoints), 6);
      }
      pdf.tagRow(tt('pdf.segment', undefined, 'Segment'), tags(n.data.segment), C_ORANGE);
  ...
  if (features.length > 0) {
    pdf.sectionTitle(tt('pdf.featureMap', undefined, 'Feature Map'));
    // Summary table
    pdf.table(
      [
        tt('nodes.feature', undefined, 'Feature'),
        tt('pdf.priority', undefined, 'Priority'),
        tt('pdf.status', undefined, 'Status'),
      ],
      features.map(n => [
  ...
  if (entities.length > 0) {
    pdf.sectionTitle(tt('pdf.dataModel', undefined, 'Data Model'));
    entities.forEach((n, i) => {
      pdf.ensureBlockFits(estimateNodeHeight(n, includeEdgeLabels));
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Untitled Entity');
      pdf.bodyText(str(n.data.description), 4);
      pdf.tagRow(tt('pdf.attribute', undefined, 'Attributes'), tags(n.data.attributes), C_GREEN);
      pdf.labelValue(tt('pdf.source', undefined, 'Source'), str(n.data.source), 4);
      pdf.labelValue(tt('pdf.owner', undefined, 'Owner'), str(n.data.owner), 4);
      if (includeEdgeLabels) pdf.connections(connMap.get(n.id) ?? { incoming: [], outgoing: [] });
    });
    pdf.spacing(6);
  }
  ...
  if (channels.length > 0) {
    pdf.sectionTitle(tt('pdf.channels', undefined, 'Channels'));
    channels.forEach((n, i) => {
      ...
      if (str(n.data.metric)) {
        pdf.labelValue(tt('fields.metric', undefined, 'Metric'), str(n.data.metric), 4);
      }
      if (str(n.data.notes)) {
  ...
  if (kpis.length > 0) {
    pdf.sectionTitle(tt('pdf.kpis', undefined, 'KPIs'));
    kpis.forEach((n, i) => {
      pdf.ensureBlockFits(estimateNodeHeight(n, includeEdgeLabels));
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Untitled KPI');
      pdf.bigMetric(str(n.data.target), str(n.data.unit));
      pdf.labelValue(tt('pdf.measurement', undefined, 'Measurement'), str(n.data.measurement), 4);
      pdf.labelValue(tt('pdf.owner', undefined, 'Owner'), str(n.data.owner), 4);
      if (includeEdgeLabels) pdf.connections(connMap.get(n.id) ?? { incoming: [], outgoing: [] });
    });
    pdf.spacing(6);
  ...
  if (risks.length > 0) {
    pdf.sectionTitle(tt('pdf.riskRegister', undefined, 'Risk Register'));
    // Summary table
    pdf.table(
      [
        tt('nodes.risk', undefined, 'Risk'),
        tt('pdf.impact', undefined, 'Impact'),
        tt('pdf.likelihood', undefined, 'Likelihood'),
        tt('pdf.status', undefined, 'Status'),
      ],
      risks.map(n => [
        str(n.data.name) || 'Untitled',
    ...
    risks.forEach(n => {
      ...
      if (str(n.data.mitigation)) {
        pdf.miniLabel(tt('pdf.mitigation', undefined, 'Mitigation'), 4);
  ...
  if (bridges.length > 0) {
    pdf.sectionTitle(tt('pdf.handoffs', undefined, 'Handoffs'));
    bridges.forEach((n, i) => {
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Handoff');
      if (str(n.data.toFlow)) {
        pdf.tagRow(tt('pdf.target', undefined, 'Target'), [str(n.data.toFlow)], C_ACCENT);
      }
      if (tags(n.data.syncFields).length > 0) {
        pdf.tagRow(tt('flowNode.syncFields', undefined, 'Sync Fields'), tags(n.data.syncFields), C_PRIMARY);
      }
      if (str(n.data.notes)) {
        pdf.bodyText(str(n.data.notes), 4);
      }
...
function renderDevelopmentSections(
  pdf: PdfBuilder,
  nodesByKind: Map<NodeKind, ProjectNode[]>,
  connMap: Map<string, ConnectionInfo>,
  includeEdgeLabels: boolean,
  ...
  if (services.length > 0) {
    pdf.sectionTitle(tt('pdf.services', undefined, 'Services'));
    pdf.table(
      [
        tt('nodes.service', undefined, 'Service'),
        tt('fields.tag', undefined, 'Tag'),
        tt('fields.endpoints', undefined, 'Endpoints'),
      ],
      services.map(n => [
        str(n.data.name) || 'Untitled',
        str(n.data.tag),
    ...
    services.forEach(n => {
      ...
      if (endpoints.length > 0) {
        pdf.table(
          [tt('pdf.method', undefined, 'Method'), tt('pdf.route', undefined, 'Route')],
  ...
  if (databases.length > 0) {
    pdf.sectionTitle(tt('pdf.databases', undefined, 'Databases'));
    databases.forEach((n, i) => {
      pdf.ensureBlockFits(estimateNodeHeight(n, includeEdgeLabels));
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Untitled Database');
      pdf.labelValue(tt('pdf.type', undefined, 'Type'), str(n.data.dbType), 4);
      ...
      if (str(n.data.schemaNotes)) {
        ...
        if (schemaRows.length > 0) {
          pdf.table(
            [tt('pdf.field', undefined, 'Field'), tt('flowNode.typeColumn', undefined, 'Type / Constraints')],
            schemaRows,
            [60, 110],
          );
        } else {
          pdf.labelValue(tt('fields.schemaNotes', undefined, 'Schema'), '', 4);
  ...
  if (integrations.length > 0) {
    pdf.sectionTitle(tt('pdf.integrations', undefined, 'Integrations'));
    pdf.table(
      [
        tt('nodes.integration', undefined, 'Integration'),
        tt('fields.boundary', undefined, 'Boundary'),
        tt('flowNode.auth', undefined, 'Auth'),
        tt('flowNode.url', undefined, 'URL'),
      ],
      integrations.map(n => [
        str(n.data.name) || 'Untitled',
  ...
  if (infra.length > 0) {
    pdf.sectionTitle(tt('pdf.infrastructure', undefined, 'Infrastructure'));
    infra.forEach((n, i) => {
      pdf.ensureBlockFits(estimateNodeHeight(n, includeEdgeLabels));
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Untitled');
      pdf.labelValue(tt('fields.provider', undefined, 'Provider'), str(n.data.provider), 4);
      pdf.noteBlock(str(n.data.notes));
      if (includeEdgeLabels) pdf.connections(connMap.get(n.id) ?? { incoming: [], outgoing: [] });
    });
    pdf.spacing(6);
  ...
  if (frameworks.length > 0) {
    pdf.sectionTitle(tt('pdf.frameworks', undefined, 'Frameworks'));
    frameworks.forEach((n, i) => {
      pdf.ensureBlockFits(estimateNodeHeight(n, includeEdgeLabels));
      if (i > 0) pdf.separator();
      pdf.subTitle(str(n.data.name) || 'Untitled');
      pdf.labelValue(tt('fields.framework', undefined, 'Framework'), str(n.data.framework), 4);
      pdf.noteBlock(str(n.data.notes));
      if (includeEdgeLabels) pdf.connections(connMap.get(n.id) ?? { incoming: [], outgoing: [] });
    });
    pdf.spacing(6);
  ...
  if (actions.length > 0) {
    pdf.sectionTitle(tt('pdf.actionsSection', undefined, 'Actions'));
    pdf.table(
      [
        tt('nodes.action', undefined, 'Action'),
        tt('fields.tag', undefined, 'Tag'),
        tt('pdf.description', undefined, 'Description'),
      ],
      actions.map(n => [
        str(n.data.name) || 'Untitled',
        str(n.data.tag),
...
function renderSharedSections(
  ...
  if (milestones.length > 0) {
    ...
    pdf.sectionTitle(tt('pdf.milestonesSection', undefined, 'Milestones'));
    pdf.table(
      [
        '#',
        tt('nodes.milestone', undefined, 'Milestone'),
        tt('pdf.priority', undefined, 'Priority'),
        tt('pdf.dueDate', undefined, 'Due Date'),
        tt('pdf.goal', undefined, 'Goal'),
      ],
      sorted.map(n => [
  ...
  if (comments.length > 0) {
    pdf.sectionTitle(tt('pdf.notes', undefined, 'Notes'));
...
export function exportProjectToPdf(project: Project, t?: TFunction) {
  const tt: PdfT = (key, params, fallback) => {
    const translated = t ? t(key, params) : key;
  ...
  const flowLabel = flow === 'business'
    ? tt('app.businessBrief', undefined, 'Business Plan')
    : tt('app.development', undefined, 'Development Architecture');
  ...
  pdf.drawCover(
    ...
    now,
    specTexts,
    tt('pdf.generatedBy', undefined, 'Generated by KnitFlow'),
  );

  // Flow-specific sections
  if (flow === 'business') {
    renderBusinessSections(pdf, nodesByKind, connMap, includeEdgeLabels, tt);
  } else {
    renderDevelopmentSections(pdf, nodesByKind, connMap, includeEdgeLabels, tt);
  }

  // Shared sections (milestones + comments only, specs already rendered on cover)
  renderSharedSections(pdf, nodesByKind, connMap, includeEdgeLabels, tt);
